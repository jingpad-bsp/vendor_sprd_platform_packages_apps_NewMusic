...
/* SPRD: bugfix 275992 play song in MusicPicker in focus strategy to solve audio conflict  @{ */
...
public class MusicPicker extends ListActivity
    ...
    class TrackListAdapter extends SimpleCursorAdapter
        ...
        public void changeCursor(Cursor cursor) {
            ...
            /* SPRD 476972 @{ */
            if (MusicPicker.this.mCursor != null && !MusicPicker.this.mCursor.isClosed()) {
    ...
    /* SPRD bugfix 516431 @{ */
    private void init() {
        MusicDRM.getInstance().initDRM(MusicPicker.this);
        /* SPRD 476972 @{ */
        registerSDListener();
    ...
    Cursor doQuery(boolean sync, String filterstring) {
        ...
        //SPRD bugfix 583255 music crashed when selecting music.
        ...
        where.append(MediaStore.Audio.Media.TITLE);
        where.append(" != '' AND ");
        where.append(MediaStore.Audio.Media.DURATION);
        where.append(" != 0");
        where.append(" AND "+MediaStore.Audio.Media.IS_MUSIC);
        where.append(" =1");
    ...
    @Override protected void onListItemClick(ListView l, View v, int position,
        ...
        boolean isDRM = MusicDRM.getInstance().isDRM(mCursor, position);
        if (isDRM) {
            showToast(R.string.error_choose_drm_for_gallery);
            return;
        } else {
            setSelected(mCursor);
        }
        ...
        if (newId != mPlayingId || mMediaPlayer == null) {
            ...
            try {
                ...
                if(audioFocus == AudioManager.AUDIOFOCUS_REQUEST_FAILED) {
                    /* SPRD bug fix 616567 The slide music can not be selected while calling. @{ */
                    //return;
                    /* @}*/
                }
                /* @} */
                mMediaPlayer.start();
                mPlayingId = newId;
                mSelectedId = newId;//SPRD bug fix 586526
                /* SPRD 607582 @{ */
                if (mSelectedId >= 0) {
    ...
    private BroadcastReceiver mScanListener = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            Log.d(TAG, "mScanListener getAction: " + action);
    ...
    protected void onDestroy() {
        ...
        /* @} */
        /*Bug 1207596:Dialog is not destroyed, which may leak window*/
