...
public class MediaButtonIntentReceiver extends BroadcastReceiver {
    ...
    private static final int MSG_TWICE_NEXT = 2;
    private static long mTwiceClickTime = 0;
    ...
    private static Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_TWICE_NEXT:
                    Context context = (Context)msg.obj;
                    Log.d(TAG,"mHandle receive twice ");
    ...
    public void onReceive(Context context, Intent intent) {
        if (ActivityManager.isUserAMonkey()) {
            Log.e(TAG, "Should not start MediaPlaybackService when system in monkey test");
            return;
        }
        if (MusicUtils.checkPermission(context) != MusicUtils.PERMISSION_ALL_ALLOWED){
            Log.d(TAG, "Permissition not allowed");
            return;
        }
        String intentAction = intent.getAction();
        /* SPRD 525433 not start MediaPlaybackService before boot complete
         * @orig:
        ...
        /* @} */
            ...
            Log.i("MediaButtonIntentReceiver", "onReceive keycode= " + keycode+"action = "+action);
            ...
            Log.i("MediaButtonIntentReceiver", "onReceive command= " + command);

            if (command != null) {
                if (MediaPlaybackService.CMDFORWARD.equals(command) ||
                                MediaPlaybackService.CMDREWIND.equals(command)) {
                    if (action == KeyEvent.ACTION_UP) {
                        MediaPlaybackService.mSkipAmount = 0;
                    }else{
                        Intent i1 = new Intent(context, MediaPlaybackService.class);
                        i1.setAction(MediaPlaybackService.SERVICECMD);
                        i1.putExtra(MediaPlaybackService.CMDNAME, command);
                        if (Build.VERSION.SDK_INT >= SDK_VERSION) {
                ...
                if (action == KeyEvent.ACTION_DOWN) {
                    if (mDown) {
                        ...
                        if (((keycode == KeyEvent.KEYCODE_HEADSETHOOK)
                                ...
                                && (eventtime - mTwiceClickTime < 300)) {
                            mHandler.removeMessages(MSG_TWICE_NEXT);
                            ...
                            Log.d(TAG,"Twice click");
                            mHandler.sendMessageDelayed(mHandler.obtainMessage(MSG_TWICE_NEXT, context), 350);
                            mLastClickTime = 0;
                            mTwiceClickTime = eventtime;
                        } else {
